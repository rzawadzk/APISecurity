-- Migration 0001: initial schema (Postgres)
--
-- This is the Postgres counterpart of 0001_initial.sqlite.sql.
-- Schema layout matches the SQLite version; only the dialect details
-- differ:
--
--   * INTEGER PRIMARY KEY AUTOINCREMENT  -->  SERIAL PRIMARY KEY
--   * REAL                               -->  DOUBLE PRECISION
--   * DEFAULT (datetime('now'))          -->  omitted; application code
--     always passes the value explicitly (see database.py timestamp
--     policy notes).
--
-- Boolean-ish columns (is_active, declared_in_spec, acknowledged) stay
-- INTEGER with 0/1 semantics so application code written for the
-- SQLite backend keeps working without coercion changes.
